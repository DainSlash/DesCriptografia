...
void initialPermutation(int blocks, int pos, int text);
...
void getKeys(int keycode){
    FILE* keyFile = (keycode==1) ? fopen("key1.txt", "rb"): (keycode==2) ? fopen("key2.txt","rb") : fopen("key3.txt","rb");
    uint8_t key[64], i=0;
    uint8_t ch;
    while(1){
        ch = getc(keyFile);
...
void encrypt_decrypt(unsigned int size, short int mode){
    FILE *inputFile;
    if( tripledes == 0)  inputFile = (mode==0) ? fopen("bits.txt","rb") :  fopen("cipher.txt","rb");//operador ternario para trocar o arquivo, dependendo do mode.
    else inputFile = (mode==0) ? fopen("decrypted.txt","rb") :  fopen("cipher.txt","rb");
    ptFILE = (mode==0) ? fopen("cipher.txt","wb+") : fopen("decrypted.txt","wb+");
    tripledes++;
    ...
    for(i=0;i<blocks;i++){//vai rodar para cada bloco de 64 bits
        ...
        for(p=(i*64);p<((i+1)*64);p++) initialPermutation(i,p,bits[p]);//envia bloco por bloco para permutar

///PEGA O TEXTO PERMUTADO INICIALMENTE, E ARMAZENA A PARTE DIREITA E ESQUERDA NO PRIMEIRO INDICE DA MATRIZ SENDO 0
        for(j = 0; j < 64; j++){
            if(j < 32) Left[0][j] = IPtext[j];
            else Right[0][j-32] = IPtext[j];
        }
///REALIZA OS 16 ROUNDS NA FUNCAO CIPHER
        for(round=1;round<17;round++){
            cipher(round,mode);
            for(j=0;j<32;j++) Left[round][j] = Right[round-1][j];
        }

///PEGA O ULTIMO ROUND DE DIREITA E ESQUERDA DEPOIS DE PASSAR POR TODOS OS ROUNDS NO CHIPHER E ARMAZENA EM UM VETOR DE 64BITS
        for(j = 0; j < 64; j++){
            if(j < 32) CIPHER[j] = Right[16][j];
            else CIPHER[j] = Left[16][j - 32];
            finalPermutation(j, CIPHER[j]);//PERMUTA OS BITS CIFRADOS E COLOCA NO VETOR FINALTEXT
        }

///ESCREVE NO PTFILE, O FINAL TEXT
        for(j=0;j<64;j++){
...
void initialPermutation(int blocks,int pos, int bit){
    int i;
    pos+=1;
    for(i=0;i<64;i++) if(IP[i] == (pos-64*blocks)) break;
    IPtext[i] = bit;
}
...
void F1(uint8_t Case){
    ...
    switch (Case){//Case é referente a qual linha da matriz XTextSBox esta sendo usada
        case 0:
